...
public class SecurityConfig  {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        //  h2 콘솔 사용등을 위해  SAMEORIGIN → 같은 출처(origin) 내에서만 iframe 허용 → localhost에서 띄우면 OK
        http.authorizeHttpRequests(authorizeRequests -> authorizeRequests.requestMatchers("/h2/**").permitAll())
                .headers(headers -> headers.frameOptions(HeadersConfigurer.FrameOptionsConfig::sameOrigin))
                .csrf(csrfConfigurer -> csrfConfigurer.ignoringRequestMatchers("/h2/**"));
        ...
        http.authorizeHttpRequests(auth -> auth
                        .requestMatchers("/assets/**", "/", "/chat/**", "/chatty/**", "/act/**", "/mnt/**", "/temp/**", "/lotto/**").permitAll()
                        .requestMatchers("/sign_up", "/sign_in", "/denied", "/errorPage").permitAll() // 무조건 허용
                        .requestMatchers("/adm/**").hasAuthority("ADMIN")
                        .requestMatchers("/ssl/**", "/pgp/**", "/crypt/**").hasAuthority("OPERATION")
