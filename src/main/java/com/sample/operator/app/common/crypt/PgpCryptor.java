...
public class PgpCryptor implements BaseCryptor {
    ...
    private PgpCollectionPair getPgpKeyColPair(String svc, String subType, Object... obj)
    {
        PgpCollectionPair pair = null;

        if( obj == null || obj.length == 0 )
        {
            System.out.println("키 관리 서버 연동이 되면 가져오고 아니면 오류로 ");
            CryptInfo ci = CryptInfo.getCryptInfo("PGP", svc, subType);
        }

        List<Object> pgpList = Arrays.asList(Objects.requireNonNull(obj));
        
        if(pgpList.size() != 2)
        {
            System.out.println("PGP 암복호화에 필요한 인자를 확인해주세요");
        }

        Object pubObj = pgpList.get(0);
        Object secObj = pgpList.get(1);

        try
        {
            PGPPublicKeyRingCollection pub = (PGPPublicKeyRingCollection) pubObj;
            PGPSecretKeyRingCollection sec = (PGPSecretKeyRingCollection) secObj;
            return new PgpCollectionPair(pub, sec);
        }
        catch (Exception e)
        {
            System.out.println("PGP 키링 형변환 실패");
            throw new OperException();
        }
    }
